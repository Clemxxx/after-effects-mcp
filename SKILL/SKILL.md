...
17. **Measuring a layout (never guess from Position values)**: A layer's Position is NOT where its pixels are — anchor point, scale, parenting and text justification all shift the visible box. Use get_layer_bounds to read the REAL comp-space boxes at a time: it returns each layer's bounds/center/offset from the canvas center, whether it is still animating at that time (animatingAtTime — if true, pick another time), the gaps between consecutive blocks along an axis (negative = overlap), the union box and the free space to the canvas edges. Typical "is this board balanced?" check = one get_layer_bounds call at a hold time; then fix with distribute_groups (spacing: N → equal N px between the EDGES of consecutive blocks, arrangement centered) and align_layers, and re-measure. Both accept area: { top, bottom, left, right } to center within the space between a header and a footer instead of the full canvas (e.g. area: { top: 200, bottom: 1780 }). Precomp layers report their frame, not their visible pixels — check inside the precomp if its content has padding.
...
- get_layer_bounds — real bounding boxes, centers, gaps between blocks and free space at a given time; the numeric way to check centering and spacing before/after align_layers / distribute_groups.
